<template>
  ...
  <mt-button icon="more" slot="right"></mt-button>
             ...
             <ul>
                <li v-for="(cart,index) in carts" class="cartList">
                     ...
                     <div class="select" @click="danxuan(cart)" >
                        <i class="iconfont icon-xuanzekuangmoren"   v-show="!cart.danx1uan">未选</i>
                        <i class="iconfont icon-xuanzekuangxuanzhong" v-show="cart.danx1uan" style="color:#25b5fe">选中</i>
                    </div>
                    ...
                    <div class="cartInformation">
                        <div class="cartName">{{cart.name}}
                            <a href="javascript:;" class="iconfont icon-huishouzhan7"  @click="shanchu(index)" >删除</a>
                        </div>
        ...
        <div class="cartImg" v-if="!carts.length">
            ...
            <h1>购物车是空的哦，快去购物吧</h1>
            <router-link :to="{name:'mail'}">逛一逛</router-link>
        </div>
